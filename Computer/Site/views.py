...
def FilterSearchMouse(request):
    ...
    if is_valid(connectint) and connectint != '----':
        qs = qs.filter(M_Mouse_ConnectionInt__ConnectionInt_id=connectint)

    if is_valid(sens) and sens != '----':
        qs = qs.filter(M_Mouse_SenserModel__M_Mouse_SenserModel_id=sens)

    if is_valid(connecttype) and connecttype != '----':
        qs = qs.filter(M_Mouse_ConnectType__ConnectType_id=connecttype)

    if is_valid(freq) and freq != '----':
        qs = qs.filter(M_Mouse_Frequency__M_Mouse_Frequency_id=freq)

    if is_valid(dpi) and dpi != '----':
        qs = qs.filter(M_Mouse_DPI__M_Mouse_DPI_id=dpi)
...
def FilterSearchKeyboard(request):
    ...
    if is_valid(connectint) and connectint != '----':
        qs = qs.filter(K_Keyboard_ConnectionInt__ConnectionInt_id=connectint)

    if is_valid(typekb) and typekb != '----':
        qs = qs.filter(K_Keyboard_Type__K_Keyboard_Type_id=typekb)

    if is_valid(connecttype) and connecttype != '----':
        qs = qs.filter(K_Keyboard_ConnectType__ConnectType_id=connecttype)

    if is_valid(formatkb) and formatkb != '----':
        qs = qs.filter(K_Keyboard_Format__K_Keyboard_Format_id=formatkb)

    if is_valid(switch) and switch != '----':
        qs = qs.filter(K_Keyboard_SwitchModel__K_Keyboard_SwitchModel_id=switch)
...
def FilterSearchHeadset(request):
    ...
    if is_valid(connectint) and connectint != '----':
        qs = qs.filter(H_HeadSet_ConnectionInt__ConnectionInt_id=connectint)

    if is_valid(schemehs) and schemehs != '----':
        qs = qs.filter(H_HeadSet_Scheme__H_HeadSet_Scheme_id=schemehs)

    if is_valid(connecttype) and connecttype != '----':
        qs = qs.filter(H_HeadSet_ConnectType__ConnectType_id=connecttype)

    if is_valid(hertz) and hertz != '----':
        qs = qs.filter(H_HeadSet_Hz__H_HeadSet_Hz_id=hertz)

    if is_valid(sens) and sens != '----':
        qs = qs.filter(H_HeadSet_Sens__H_HeadSet_Sens_id=sens)

    if is_valid(seths) and seths != '----':
        qs = qs.filter(H_HeadSet_Set__H_HeadSet_Set_id=seths)
