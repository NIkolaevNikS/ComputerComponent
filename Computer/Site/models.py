...
# Модель таблицы типа подключения
...
# Модель таблицы интерфейса подключения
...
# Модель таблицы частоты обновления компьютерных мышей
...
# Модель таблицы максимального разрешение датчика компьютерных мышей
class M_Mouse_DPI(models.Model):
    """Модель таблицы модели сенсора компьютерных мышей"""
...
# Модель таблицы компьютерных мышей
class M_Mouse(models.Model):
    ...
    M_Mouse_Image = models.ImageField(null=True, blank=True, verbose_name="Изображение модели")
    M_Mouse_Price = models.DecimalField(max_digits=19, decimal_places=0, verbose_name="Цена компьютерной мышки",
                                        null=True, blank=True)

    def mouse_image_set(self):
        """Функция передачи изображения из модели"""
    ...
    def get_absolute_url(self):
        """Функция передачи ссылки на запись по идентификатору"""
...
# Модель таблицы модели переключателя клавиатур
...
# Модель таблицы формата клавиатур
...
class K_Keyboard(models.Model):
    ...
    K_Keyboard_Price = models.DecimalField(max_digits=19, decimal_places=0, verbose_name="Цена клавиатуры", null=True,
                                           blank=True)
...
# Модель таблицы схемы звука наушников
class H_HeadSet_Scheme(models.Model):
    """Модель таблицы схемы звука наушников"""
    H_HeadSet_Scheme_id = models.AutoField(primary_key=True, help_text="ID Схемы", unique=True)# Модель таблицы схемы звука наушников
...
# Модель таблицы воспроизводимой частоты наушников
...
# Модель таблицы чувствительности наушников
...
# Модель таблицы комплекта наушников
...
class H_HeadSet(models.Model):
    ...
    H_HeadSet_Price = models.DecimalField(max_digits=19, decimal_places=0, verbose_name="Цена клавиатуры", null=True,
                                          blank=True)
    ...
    def headset_image_set(self):
        """Функция вывода изображения на страницу записей о наушниках"""
    ...
    def headset_image_set1(self):
        """Функция вывода изображения на страницу интерфейса администратора"""
